    ...
    _.each(contentTypes, (pages, contentType) => {
      ...
      if (contentType === "tours") {
        ...
        allRegions.forEach(element => {
          ...
          allTagsRegion.forEach(item => {
            if (element) {
              thisSlug = "/"+element.toLowerCase()+"/"+item.toLowerCase()+"/"
            } else {
              thisSlug = "/tours/"+item.toLowerCase()+"/"
            }
            template = "ToursIndexPage4" //ToursIndexPage2 shows correct tours, + MUI cards
          ...
          template = "ToursIndexPage4reg" //removes tag from graphql
